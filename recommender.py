...
def recommend_music(article_url, music_data=response_music):
    ...
    article_sentiments, article_content, article_themes = article_analysis.relevant_info(article_url)
    # for each lyric
    for idx, row in music_data.iterrows():
        lyrics = row['lyrics']
        music_tokens, music_freq_dist, music_sentiment_scores = music_analysis.extract_features_from_lyrics(lyrics)
        
        # Calculate similarity score
        similarity_score = calculate_similarity(article_content, music_tokens)
        
        # Compare sentiment scores
        sentiment_difference = abs(article_sentiments[0] - music_sentiment_scores['compound'])
