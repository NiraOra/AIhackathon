...
import music_analysis as music_analysis
import pandas as pd
import article_analysis as article_analysis

# from kaggle dataset: https://www.kaggle.com/datasets/saurabhshahane/music-dataset-1950-to-2019
...
def recommend_music(article_url, music_data=response_music):
    ...
    (article_polarity, article_sentiment), article_content = article_analysis.relevant_info(article_url)
    
    # for each lyric
    for idx, row in music_data.iterrows():
        lyrics = row['lyrics']
        (music_polarity, music_sentiment) = music_analysis.extract_features_from_lyrics(lyrics)
        # Add genre as a feature
        music_features = [lyrics] + [row['genre']] 
        ...
        # combined_score = similarity_score * sentiment weight (to check against polarity) - (difference of sentimentality)
